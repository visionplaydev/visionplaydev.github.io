...
(function(){
  ...
  // 활성 팝업들을 순서대로 (닫으면 다음). '오늘 하루 보지 않기' 체크 시 큐 전체 중단.
  function showQueue(list, idx){
    if(idx >= list.length) return;
    render(list[idx], function(goNext){ if(goNext) showQueue(list, idx + 1); });
  }
  ...
  function render(c, done){
    ...
    var dismissToday=false, closed=false;
    function close(){
      ...
      if(done) done(!dismissToday);   // 오늘안보기면 큐 중단, 아니면 다음 팝업
    }
